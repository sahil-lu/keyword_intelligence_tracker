	...
	return (
		<DialogPortal>
			...
			<DialogPrimitive.Content
				...
				className={cn(
					"bg-oklch(1 0 0) text-oklch(0.145 0 0) ring-oklch(0.145 0 0)/10 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 dark:bg-oklch(0.205 0 0) dark:text-oklch(0.985 0 0) dark:ring-oklch(0.985 0 0)/10 fixed top-1/2 left-1/2 z-50 grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl p-4 text-sm ring-1 duration-100 outline-none sm:max-w-sm",
					className
				)}
				{...props}
			>
...
function DialogFooter({ className, dismissible = false, children, ...props }) {
	return (
		<div
			...
			className={cn(
				"bg-oklch(0.97 0 0)/50 dark:bg-oklch(0.269 0 0)/50 -mx-4 -mb-4 flex flex-col-reverse gap-2 rounded-b-xl border-t p-4 sm:flex-row sm:justify-end",
				className
			)}
			{...props}
		>
